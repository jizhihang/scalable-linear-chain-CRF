function logLL = crf_getRLLD(dataList,lambda)
numData = size(dataList,2);
logLL = 0;
for n = 1:numData
    data = dataList{1,n};
    logSrcDensity = data.logSrcDensity;
    logTrgDensity = data.logTrgDensity;
    weight = exp(logSrcDensity - logTrgDensity);
    eFeature = data.eFeature;
    rFCellMatrList = data.rFCellMatrList;
    cardY = size(rFCellMatrList{1,1},1);
    T = size(rFCellMatrList,2);
    mList = getMList(rFCellMatrList,cardY,T,weight*lambda);
    alphaTList = getAlphaTList(mList,cardY,T); 
    logZ = crf_getLogSum(alphaTList{1,T});
    logLL = logLL+weight*lambda'*eFeature-logZ;
end
end
